...
from apresentacaoDeTrabalho import ApresentacaoDeTrabalho

from areaDeAtuacao import AreaDeAtuacao
from artigoAceito import ArtigoAceito
from artigoEmPeriodico import ArtigoEmPeriodico
from capituloDeLivroPublicado import CapituloDeLivroPublicado
from desenhoIndustrial import DesenhoIndustrial
from formacaoAcademica import FormacaoAcademica
from idioma import Idioma
from livroPublicado import LivroPublicado
from organizacaoDeEvento import OrganizacaoDeEvento
from orientacaoConcluida import OrientacaoConcluida
from orientacaoEmAndamento import OrientacaoEmAndamento
from outroTipoDeProducaoBibliografica import OutroTipoDeProducaoBibliografica
from outroTipoDeProducaoTecnica import OutroTipoDeProducaoTecnica
from participacaoEmEvento import ParticipacaoEmEvento
from patente import Patente
from premioOuTitulo import PremioOuTitulo
from processoOuTecnica import ProcessoOuTecnica
from producaoArtistica import ProducaoArtistica
from produtoTecnologico import ProdutoTecnologico
from programaComputador import ProgramaComputador
from projetoDePesquisa import ProjetoDePesquisa
from resumoEmCongresso import ResumoEmCongresso
from resumoExpandidoEmCongresso import ResumoExpandidoEmCongresso
from softwareComPatente import SoftwareComPatente
from softwareSemPatente import SoftwareSemPatente
from textoEmJornalDeNoticia import TextoEmJornalDeNoticia
from trabalhoCompletoEmCongresso import TrabalhoCompletoEmCongresso
from trabalhoTecnico import TrabalhoTecnico
...
class ParserLattes(HTMLParser):
        ...
        if tag == 'a':
            if self.salvarItem:  # and self.achouArtigoEmPeriodico:
                for name, value in attributes:
                    ...
                    id_val = re.findall(u'http://lattes.cnpq.br/(\d{16})', value)
                    if name == 'href' and len(id_val) > 0:
                        self.listaIDLattesColaboradores.append(id_val[0])
                        if self.achouOrientacoesEmAndamento or self.achouOrientacoesConcluidas:
                            self.idOrientando = id_val[0]
                        break
    ...
    def handle_endtag(self, tag):
        ...
        if tag == 'span' and self.salvarBolsaProdutividade:
            ...
            self.bolsaProdutividade = re.sub('Bolsista de Produtividade em Pesquisa do CNPq - ', '',
                                             self.bolsaProdutividade)
            self.bolsaProdutividade = self.bolsaProdutividade.strip('()')
            self.salvarBolsaProdutividade = 0

        if tag == 'span' and self.salvarIdentificador16 == 1:
            # self.identificador16 = re.findall(u'http://lattes.cnpq.br/(\d{16})', value) # FIXME: var nao declarada: value
            # @Kepler
            value = self.item
                ...
                if self.salvarParte2:
                    ...
                    if self.achouFormacaoAcademica and len(self.partesDoItem) >= 2:
                        ...
                        self.listaFormacaoAcademica.append(
                            iessimaFormacaoAcademica)  # acrescentamos o objeto de FormacaoAcademica
                        ...
                        if not self.salvarParte3:
                            ...
                            if len(self.partesDoItem) >= 3:
                                ...
                                self.listaProjetoDePesquisa.append(
                                    iessimoProjetoDePesquisa)  # acrescentamos o objeto de ProjetoDePesquisa
                    ...
                    if self.achouPremioOuTitulo and len(self.partesDoItem) >= 2:
                        iessimoPremio = PremioOuTitulo(self.idMembro,
                                                       self.partesDoItem)  # criamos um objeto com a lista correspondentes às celulas da linha
                    ...
                    if self.achouProducoes:
                        if self.achouProducaoEmCTA:
                            if self.achouArtigoEmPeriodico:
                                iessimoItem = ArtigoEmPeriodico(self.idMembro,
                                                                self.partesDoItem,
                                                                self.doi,
                                                                self.relevante,
                                                                self.issn)
                            ...
                            if self.achouLivroPublicado:
                                iessimoItem = LivroPublicado(self.idMembro,
                                                             self.partesDoItem,
                                                             self.relevante)
                            ...
                            if self.achouTrabalhoCompletoEmCongresso:
                                iessimoItem = TrabalhoCompletoEmCongresso(
                                    self.idMembro, self.partesDoItem, self.doi, self.relevante)
                            ...
                            if self.achouResumoEmCongresso:
                                iessimoItem = ResumoEmCongresso(self.idMembro,
                                                                self.partesDoItem,
                                                                self.doi,
                                                                self.relevante)
                            ...
                            if self.achouArtigoAceito:
                                iessimoItem = ArtigoAceito(self.idMembro,
                                                           self.partesDoItem,
                                                           self.doi,
                                                           self.relevante)
                        ...
                        if self.achouProducaoArtisticaCultural:
                            if self.achouOutraProducaoArtisticaCultural:
                                iessimoItem = ProducaoArtistica(self.idMembro,
                                                                self.partesDoItem,
                                                                self.relevante)
                        ...
                        if self.achouOrientacoesEmAndamento:
                            if self.achouSupervisaoDePosDoutorado:
                                self.listaOASupervisaoDePosDoutorado.append(
                                    OrientacaoEmAndamento(self.idMembro,
                                                          self.partesDoItem,
                                                          self.idOrientando))
                                self.idOrientando = ''
                        ...
                        if self.achouOrientacoesConcluidas:
                            if self.achouSupervisaoDePosDoutorado:
                                self.listaOCSupervisaoDePosDoutorado.append(
                                    OrientacaoConcluida(self.idMembro,
                                                        self.partesDoItem,
                                                        self.idOrientando))
                                self.idOrientando = ''
                            if self.achouTeseDeDoutorado:
    ...
    def handle_data(self, dado):
        if not self.spanInformacaoArtigo:
            self.item += htmlentitydecode(dado)
        ...
        if self.recuperarIdentificador16 and self.identificador16 == '':
            id_ = re.findall(u'http://lattes.cnpq.br/(\d{16})', dado)
            if len(id_) > 0:
                self.identificador16 = id_[0]
