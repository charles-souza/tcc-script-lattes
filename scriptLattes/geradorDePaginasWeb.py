...
from scriptLattes.charts.graficoDeInternacionalizacao import GraficoDeInternacionalizacao
...
class GeradorDePaginasWeb:
    ...
    def gerarPaginaPrincipal(self):
        ...
        if self.nPR0 > 0:
            s += '<li> <a href="PR0-0' + self.extensaoPagina + '">Patente</a> '.decode("utf8") + '(' + str(
                self.nPR0) + ')'
        if self.nPR1 > 0:
            s += '<li> <a href="PR1-0' + self.extensaoPagina + '">Programa de computador</a> '.decode(
                "utf8") + '(' + str(self.nPR1) + ')'
        if self.nPR2 > 0:
            s += '<li> <a href="PR2-0' + self.extensaoPagina + '">Desenho industrial</a> '.decode("utf8") + '(' + str(
                self.nPR2) + ')'
    ...
    def gerarPaginasDeProducoesBibliograficas(self):
        ...
        if self.grupo.obterParametro('relatorio-incluir_artigo_em_periodico'):
            self.nPB0 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaArtigoEmPeriodico,
                                                    "Artigos completos publicados em periódicos", "PB0", ris=True)
        if self.grupo.obterParametro('relatorio-incluir_livro_publicado'):
            self.nPB1 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaLivroPublicado,
        ...
        if self.grupo.obterParametro('relatorio-incluir_capitulo_de_livro_publicado'):
            self.nPB2 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaCapituloDeLivroPublicado,
                                                    "Capítulos de livros publicados", "PB2", ris=True)
        if self.grupo.obterParametro('relatorio-incluir_texto_em_jornal_de_noticia'):
            self.nPB3 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaTextoEmJornalDeNoticia,
        ...
        if self.grupo.obterParametro('relatorio-incluir_trabalho_completo_em_congresso'):
            self.nPB4 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaTrabalhoCompletoEmCongresso,
        ...
        if self.grupo.obterParametro('relatorio-incluir_resumo_expandido_em_congresso'):
            self.nPB5 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaResumoExpandidoEmCongresso,
        ...
        if self.grupo.obterParametro('relatorio-incluir_resumo_em_congresso'):
            self.nPB6 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaResumoEmCongresso,
        ...
        if self.grupo.obterParametro('relatorio-incluir_artigo_aceito_para_publicacao'):
            self.nPB7 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaArtigoAceito,
                                                    "Artigos aceitos para publicação", "PB7")
        if self.grupo.obterParametro('relatorio-incluir_apresentacao_de_trabalho'):
            self.nPB8 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaApresentacaoDeTrabalho,
                                                    "Apresentações de trabalho", "PB8")
        if self.grupo.obterParametro('relatorio-incluir_outro_tipo_de_producao_bibliografica'):
            self.nPB9 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOutroTipoDeProducaoBibliografica,
                                                    "Demais tipos de produção bibliográfica", "PB9")
        # Total de produção bibliográfica
        self.nPB = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaPB, "Total de produção bibliográfica",
                                               "PB")
    ...
    def gerarPaginasDeProducoesTecnicas(self):
        ...
        if self.grupo.obterParametro('relatorio-incluir_software_com_patente'):
            self.nPT0 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaSoftwareComPatente,
                                                    "Softwares com registro de patente", "PT0")
        if self.grupo.obterParametro('relatorio-incluir_software_sem_patente'):
            self.nPT1 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaSoftwareSemPatente,
                                                    "Softwares sem registro de patente", "PT1")
        if self.grupo.obterParametro('relatorio-incluir_produto_tecnologico'):
            self.nPT2 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaProdutoTecnologico,
                                                    "Produtos tecnológicos", "PT2")
        if self.grupo.obterParametro('relatorio-incluir_processo_ou_tecnica'):
            self.nPT3 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaProcessoOuTecnica,
                                                    "Processos ou técnicas", "PT3")
        if self.grupo.obterParametro('relatorio-incluir_trabalho_tecnico'):
            self.nPT4 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaTrabalhoTecnico,
                                                    "Trabalhos técnicos", "PT4")
        if self.grupo.obterParametro('relatorio-incluir_outro_tipo_de_producao_tecnica'):
            self.nPT5 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOutroTipoDeProducaoTecnica,
    ...
    def gerarPaginasDeProducoesArtisticas(self):
        ...
        if self.grupo.obterParametro('relatorio-incluir_producao_artistica'):
            self.nPA0 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaProducaoArtistica,
                                                    "Produção artística/cultural", "PA0")
        # Total de produções técnicas
        self.nPA = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaPA, "Total de produção artística",
                                               "PA")
    ...
    def gerarPaginasDePatentes(self):
        ...
        if self.grupo.obterParametro('relatorio-incluir_patente'):
            ...
            self.nPR1 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaProgramaComputador,
                                                    "Programa de computador", "PR1")
            self.nPR2 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaDesenhoIndustrial,
                                                    "Desenho industrial", "PR2")

        # Total de produções técnicas
        self.nPR = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaPR, "Total de patentes e registros",
                                               "PR")
    ...
    def gerarPaginasDeOrientacoes(self):
        ...
        if self.grupo.obterParametro('relatorio-incluir_orientacao_em_andamento_pos_doutorado'):
            self.nOA0 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOASupervisaoDePosDoutorado,
                                                    "Supervisão de pós-doutorado", "OA0")
        if self.grupo.obterParametro('relatorio-incluir_orientacao_em_andamento_doutorado'):
            self.nOA1 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOATeseDeDoutorado,
                                                    "Tese de doutorado", "OA1")
        if self.grupo.obterParametro('relatorio-incluir_orientacao_em_andamento_mestrado'):
            self.nOA2 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOADissertacaoDeMestrado,
        ...
        if self.grupo.obterParametro('relatorio-incluir_orientacao_em_andamento_monografia_de_especializacao'):
            self.nOA3 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOAMonografiaDeEspecializacao,
                                                    "Monografia de conclusão de curso de aperfeiçoamento/especialização",
                                                    "OA3")
        if self.grupo.obterParametro('relatorio-incluir_orientacao_em_andamento_tcc'):
            self.nOA4 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOATCC,
        ...
        if self.grupo.obterParametro('relatorio-incluir_orientacao_em_andamento_iniciacao_cientifica'):
            self.nOA5 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOAIniciacaoCientifica,
                                                    "Iniciação científica", "OA5")
        if self.grupo.obterParametro('relatorio-incluir_orientacao_em_andamento_outro_tipo'):
            self.nOA6 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOAOutroTipoDeOrientacao,
                                                    "Orientações de outra natureza", "OA6")
        # Total de orientações em andamento
        self.nOA = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOA,
        ...
        if self.grupo.obterParametro('relatorio-incluir_orientacao_concluida_pos_doutorado'):
            self.nOC0 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOCSupervisaoDePosDoutorado,
                                                    "Supervisão de pós-doutorado", "OC0")
        if self.grupo.obterParametro('relatorio-incluir_orientacao_concluida_doutorado'):
            self.nOC1 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOCTeseDeDoutorado,
                                                    "Tese de doutorado", "OC1")
        if self.grupo.obterParametro('relatorio-incluir_orientacao_concluida_mestrado'):
            self.nOC2 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOCDissertacaoDeMestrado,
        ...
        if self.grupo.obterParametro('relatorio-incluir_orientacao_concluida_monografia_de_especializacao'):
            self.nOC3 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOCMonografiaDeEspecializacao,
                                                    "Monografia de conclusão de curso de aperfeiçoamento/especialização",
                                                    "OC3")
        if self.grupo.obterParametro('relatorio-incluir_orientacao_concluida_tcc'):
            self.nOC4 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOCTCC,
        ...
        if self.grupo.obterParametro('relatorio-incluir_orientacao_concluida_iniciacao_cientifica'):
            self.nOC5 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOCIniciacaoCientifica,
                                                    "Iniciação científica", "OC5")
        if self.grupo.obterParametro('relatorio-incluir_orientacao_concluida_outro_tipo'):
            self.nOC6 = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOCOutroTipoDeOrientacao,
                                                    "Orientações de outra natureza", "OC6")
        # Total de orientações concluídas
        self.nOC = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOC, "Total de orientações concluídas",
                                               "OC")


    def gerarPaginasDeProjetos(self):
        self.nPj = 0
        self.nPj = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaProjetoDePesquisa,
                                               "Total de projetos de pesquisa", "Pj")


    def gerarPaginasDePremios(self):
        self.nPm = 0
        self.nPm = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaPremioOuTitulo,
                                               "Total de prêmios e títulos", "Pm")

    def gerarPaginasDeParticipacaoEmEventos(self):
        self.nEp = 0
        self.nEp = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaParticipacaoEmEvento,
                                               "Total de participação em eventos", "Ep")

    def gerarPaginasDeOrganizacaoDeEventos(self):
        self.nEo = 0
        self.nEo = self.gerarPaginaDeProducoes(self.grupo.compilador.listaCompletaOrganizacaoDeEvento,
        ...
        if len(keys) > 0:  # apenas geramos páginas web para lista com pelo menos 1 elemento
            ...
            for ano in keys:
                ...
                for index in range(0, len(elementos)):
                    pub = elementos[index]
                    s += '<tr valign="top"><td>' + str(index + 1) + '. &nbsp;</td> <td>' + pub.html(
                        self.grupo.listaDeMembros) + '</td></tr>'
            ...
            if self.grupo.obterParametro('grafo-considerar_rotulos_dos_membros_do_grupo'):
                for i in range(0, len(self.grupo.listaDeRotulos)):
                    ...
                    for j in range(0, len(ranks)):
                        if rotulos[j] == rot:
                            s += '<tr><td>' + str(round(ranks[j], 2)) + '</td><td>' + autores[j] + '</td></tr>'
                            somaAuthorRank += ranks[j]
    ...
    def salvarPagina(self, nome, conteudo):
        file_ = open(self.dir + "/" + nome, 'w')
        file_.write(conteudo.encode('utf8', 'replace'))
        file_.close()
