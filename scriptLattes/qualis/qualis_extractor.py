...
class qualis_extractor(object):
    ...
    def parseContent(self, html):
        ...
        if tmp is not None: tmp = tmp.find('tbody')  # get the body of the table
        if tmp is not None:  # get all rows
            tmp = tmp.findAll('tr')
        else:
            return None
